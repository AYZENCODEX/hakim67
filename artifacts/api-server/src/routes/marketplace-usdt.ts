...
const usdtListingOwnerOrAdminEngine = new PolicyEngine({ onDecision: pepDecisionObserver });
usdtListingOwnerOrAdminEngine.registerRule("resource-ownership", createResourceOwnershipRule());
usdtListingOwnerOrAdminEngine.registerRule("role-override", createRoleOverrideRule(["admin"]));
  ...
  try {
    ...
    try {
      ...
      const debit = await client.query(
        "UPDATE marketplace_wallets SET balance=balance-$1, updated_at=NOW() WHERE user_id=$2 AND market_type='usdt' AND balance >= $1",
        [cost, buyerId]
      );
      if (debit.rowCount === 0) {
      ...
      if (remaining <= 0) {
        await client.query("UPDATE usdt_listings SET status='sold', buyer_id=$1, sold_at=NOW() WHERE id=$2", [buyerId, listing_id]);
      } else {
        await client.query(
...
router.delete("/marketplace/usdt/listings/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  try {
    ...
    const ownership = await authorize({
      req,
      engine: usdtListingOwnerOrAdminEngine,
      action: "marketplace.usdt_listing.cancel",
      resource: { type: "marketplace.usdt_listing", id, ownerId: listing.seller_id },
    });
